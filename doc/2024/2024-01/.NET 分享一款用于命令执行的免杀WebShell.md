...
专攻.NET安全的  dotNet安全矩阵   2024-01-24 08:35  
  
01  
  
免杀的WebShell  
  
在.NET领域可以将这个漏洞修改成Webshell，巧妙地利用了base64编码和yso生成的特性，成功实现了免杀。  
通过构造特定的HTTP请求，可以执行恶意命令，例如启动系统进程（如winver），从而在目标服务器上执行攻击者所期望的操作。  
  
  
  
![](https://mmbiz.qpic.cn/mmbiz_jpg/NO8Q9ApS1Y9KWD6XKvicKGLKLmriadxfLFnIq3pH5C8ovBibrH5lC5AKfPLomZEdO5O079fXVZgMTXScZQexTHrUQ/640?wx_fmt=jpeg&from=appmsg "")  
