...
这些漏洞可以使攻击者通过控制恶意服务器，读取、写入任何已连接的客户端文件，包括但不限于提取敏感信息（如 SSH 密钥）或覆盖用户的配置文件（如 ~/.bashrc  
 和 ~/.popt  
），从而执行恶意代码。  
...
- **CVE-2024-12088 (CVSS 6.5)：** 使用 --safe-links  
...
1. **针对 CVE-2024-12084：** 通过使用 CFLAGS=-DDISABLE_SHA512_DIGEST  
 和 CFLAGS=-DDISABLE_SHA256_DIGEST  
 禁用 SHA 算法支持，降低该漏洞的风险。  
  
1. **针对 CVE-2024-12085：** 在编译时使用 -ftrivial-auto-var-init=zero  
