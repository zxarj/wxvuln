...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Tb6OwBlojEic7ubWs258IrIfAkCZd5ibsfhT09W0mz2tv4IUA8DiaZ1jVc19V6wHkOBibYrI5VkIXmoojhFcEuXGicA/640?wx_fmt=png&from=appmsg "")  
#### 二、更改支付状态  
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Tb6OwBlojEic7ubWs258IrIfAkCZd5ibsf3dyXxbqZVYZgTPJ3demnG9RibWPQ2N5gmhvcvWk9XvYwJkMntcTDiaUw/640?wx_fmt=png&from=appmsg "")  
#### 四、更改订单信息  
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Tb6OwBlojEic7ubWs258IrIfAkCZd5ibsfEpiciaSKLqcmPugGabuxzCXwjGu2sEy7tO2TcxAKfknPwmysJzDp3h4g/640?wx_fmt=png&from=appmsg "")  
  
2、修改订单号  
  
将金额不同的订单进行替换，可以支付一个金额较少的订单，然后将订单号修改为金额较大的订单，少付实际金额。  
  
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Tb6OwBlojEic7ubWs258IrIfAkCZd5ibsfMFWsHl0D1JU5Eb9V1FrBg48iaselRB7fbZKsiaMTH2ktcKf7c8vC5qHw/640?wx_fmt=png&from=appmsg "")  
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Tb6OwBlojEic7ubWs258IrIfAkCZd5ibsfuFyBapZdff02fZRy5bVcibyED6mo9LBn4VdAr5lVOZUyAIXsJ7vmSnA/640?wx_fmt=png&from=appmsg "")  
  
2、未对负数做检验的还可以将数量改为负数。（这里需要注意，因为后端大部分会校验不允许实付金额小于0或者0.01等，所以有的时候要想实现订单成功生成需要结合实际修改价格）  
  
生成订单时有参数表示商品数量，修改为-1  
  
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Tb6OwBlojEic7ubWs258IrIfAkCZd5ibsfNleP0pFJOpf5oQgKvdg5rv1Wq4cwK0oSPHicjEW0ZJGa87pldrHpAcg/640?wx_fmt=png&from=appmsg "")  
  
修改数量为-1后会发现，此时金额为负数。  
  
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Tb6OwBlojEic7ubWs258IrIfAkCZd5ibsfCM4yVGOf4DUHDxTAegktWbkyzHOXJWlVyiaxP84z5aic0gjOs1CaNFTA/640?wx_fmt=png&from=appmsg "")  
  
在提交订单支付的时候，为保证支付成功需要修改金额。  
  
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Tb6OwBlojEic7ubWs258IrIfAkCZd5ibsfgVZT05DmUqQbMBRnmSfBtRjqW1JWiaXHO7mIE0IkiaVaaWHQTqQpDzKw/640?wx_fmt=png&from=appmsg "")  
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Tb6OwBlojEic7ubWs258IrIfAkCZd5ibsf6iakezyeWqCeXvrRdpliaeG0sxnOsQQYApZk5DW3zYonlCHibs6YYdnsg/640?wx_fmt=png&from=appmsg "")  
  
4、手动增加订单中商品相关的多个参数以达到少付多买的目的。  
  
有的时候在提交订单时抓取数据包可以看到只有一套商品的信息，尝试多添加几套同样的参数订单是否会有变化。  
  
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Tb6OwBlojEic7ubWs258IrIfAkCZd5ibsfOVEU914iaTbmFggyFaPicEwCfjiaT1tkMX0PzE9QFic1GGe580cM1hG5icg/640?wx_fmt=png&from=appmsg "")  
  
尝试在提交订单的时候多添加几个此类参数  
  
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Tb6OwBlojEic7ubWs258IrIfAkCZd5ibsfbRjpo5cx7EbxwIBzfogqZMq9P8e45qGhIb9bZjxQ3o47OIp5zteJicg/640?wx_fmt=png&from=appmsg "")  
  
  
  
提交订单实际支付金额未变仍是一个商品的价格，但是实际套餐已经变成了四个。  
  
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Tb6OwBlojEic7ubWs258IrIfAkCZd5ibsfVEuUbwQI55nljWnC0BZcP52fuQaTF91zEDTFkysHicgGsqdVh927IdA/640?wx_fmt=png&from=appmsg "")  
#### 六、重复支付，突破限购  
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Tb6OwBlojEic7ubWs258IrIfAkCZd5ibsfG74PPPrDNKrJAepydYzHtwibGxHrNev9IHSVTvE1DBQiasgMu103Pvcg/640?wx_fmt=png&from=appmsg "")  
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/Tb6OwBlojEic7ubWs258IrIfAkCZd5ibsf1THhKzuicWTibKIxR1iaVicKEpjfAocdzN7ebQuYBa0iav2qpAmH9eaXF6w/640?wx_fmt=png&from=appmsg "")  
#### 九、利用小数点精度四舍五入  
